...
contract Pair is IPair {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (address _token0, address _token1) = (token0, token1);
        if (amount0In > 0) _sendTokenFees(token0, amount0In * IPairFactory(factory).getFee(stable) / 10000);
        if (amount1In > 0) _sendTokenFees(token1, amount1In * IPairFactory(factory).getFee(stable) / 10000);
